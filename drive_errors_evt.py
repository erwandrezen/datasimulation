...
f_errors    = json.load(open(sys.argv[1]))
...
# we read the file to be modified
    ...
    for f in range(nb_files):
        ...
        for e,evt in enumerate(l_scenarios[s]["evt"]):
            ...
            for p in range(err):
                ...
                if evt['err']=='yob':
                    if evt['typ_err']=='null':  df_err.loc[df_err['num_pat']==numpat,'yob'] = None
                    if evt['typ_err']==''    :  df_err.loc[df_err['num_pat']==numpat,'yob'] = 2030
                        
                if evt['err']=='mob':
                    if evt['typ_err']=='null':  df_err.loc[df_err['num_pat']==numpat,'mob'] = None
                    if evt['typ_err']==''    :  df_err.loc[df_err['num_pat']==numpat,'mob'] = '99'    
