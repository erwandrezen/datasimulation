...
def gen_new_pat_init (num_pat,sex,age_mean,age_sd,ref_year,delay_death):
    ...
    new_pat= [num_pat,sex,yob,mob,com,init_dte,death,death_dte,l_init['mark'],init_dte]
    evt_pat.append(new_pat)
    return evt_pat
...
def gen_new_pat (num_pat,sex,age_mean,age_sd,ref_year):
   ...
   # new_pat=[num_pat,sex,yob,mob,com,init_dte,death,death_dte]
    ...
    new_pat= [num_pat,sex,yob,mob,com,init_dte,death,death_dte,'INIT',init_dte]
    evt_pat.append(new_pat)         
    return evt_pat
...
df_init["death_ts"] = (pd.to_datetime(df_init["death_dte"], format="%Y-%m-%d")-pd.to_datetime('1900-01-01', format="%Y-%m-%d")).dt.days
df_init["mark_ts"] = (pd.to_datetime(df_init["mark_dte"], format="%Y-%m-%d")-pd.to_datetime('1900-01-01', format="%Y-%m-%d")).dt.days
df_init["birth_ts"] = (pd.to_datetime(df_init["yob"].map(str)+df_init["mob"].map(str)+'01', format="%Y%m%d")-pd.to_datetime('1900-01-01', format="%Y-%m-%d")).dt.days
...
df_healthy["death_ts"] = (pd.to_datetime(df_healthy["death_dte"], format="%Y-%m-%d")-pd.to_datetime('1900-01-01', format="%Y-%m-%d")).dt.days
df_healthy["mark_ts"] = (pd.to_datetime(df_healthy["mark_dte"], format="%Y-%m-%d")-pd.to_datetime('1900-01-01', format="%Y-%m-%d")).dt.days
